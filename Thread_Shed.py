...
Unfortunately, the Thread Shed has an extremely out-dating register system and stores all of the transaction information in
one huge unwieldy string called daily_sales.
...
for word2 in daily_transactions_split:
  for x in range(0,4):
    word2[x]=word2[x].strip()
  transactions_clean.append(word2) 
customers=[]
sales=[]
thread_sold=[]

for word3 in transactions_clean:
  customers.append(word3[0])
  sales.append(word3[1])
  thread_sold.append(word3[2])
total_sales=0 

for cash in sales:
  total_sales+=float(cash.strip('$'))
thread_sold_split=[]
...
for color in thread_sold:
  if color.find('&')==-1:
    ...
    aux=color.split('&')
    thread_sold_split.append(aux[0])
    thread_sold_split.append(aux[1])

def color_count(color):
  cuenta=0
  for df in thread_sold_split:
    if df==color:
      cuenta+=1
  return cuenta
...
for f in colors:
  valor=color_count(f)
  sentence='Today we sell {val} items of the color {col}\n'.format(val=valor,col=f)
  print(sentence)
